...
const Navbar = () => {
  ...
  const getResponsiveNavLinkClass = ({ isActive }) =>
    isActive ? "text-white" : "text-black";
  const responsivNav = (
    <div className="flex flex-col gap-2">
      <>
        <NavLink to={"/"} className={getResponsiveNavLinkClass}>
          Home
        </NavLink>
        <NavLink to={"allBooks"} className={getResponsiveNavLinkClass}>
          All books
        </NavLink>
        <NavLink to={"contact"} className={getResponsiveNavLinkClass}>
          Contact
        </NavLink>
      </>
    </div>
  ...
  return (
    <div className="navbar mx-auto bg-[#ffffff] fixed left-0 right-0 z-50 top-0 shadow-2xl">
      <div className="navbar-start">
        <div>
          <FaBarsStaggered
            className="relative block ml-2 sm:hidden"
            onClick={toggleMenu}
          />
          {isMenuVisible && (
        ...
        <Link to={"/"}>
          <img
            className="ml-10"
            src="https://i.ibb.co/prZxgTGx/Screenshot-8.png"
            alt=""
          />
        </Link>
        <input type="text" />
      ...
      <div className="navbar-end">
        {/* profile picture clicking interface */}
        ...
        {user ? (
          <>
            ...
            <h1 className=" hidden sm:block mr-5 uppercase font-bold">
              {user.displayName}
            </h1>
            <div className="relative mr-2 sm:mr-16">
              <img
                className="cursor-pointer h-6 w-6 sm:h-8 sm:w-8 rounded-full "
              ...
              {isDropdownVisible && (
                <div className="absolute z-10 bg-cyan-400 rounded shadow-4xl w-44  p-5 mt-2 right-0 bg-opacity-90 sm:w-52">
                  ...
                  <div className="">
                    <h1 className="text-center mt-2 text-lg font-bold text-white uppercase">
                      {user.displayName}
                    </h1>
                  </div>
                  <hr className="my-5" />
                  ...
                  <Link onClick={handleLogOut} to={"/"}>
                    <button className="btn rounded btn-sm border-none mt-2 bg-red-500 text-white">
                      Log Out <FiLogOut />
                    </button>
                  </Link>
                </div>
